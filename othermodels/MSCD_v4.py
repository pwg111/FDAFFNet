...
# Cross_time_level Fusion module
class CTLFM(nn.Module):
    ...
    def forward(self, feat1, feat2, pred=None):
        feat_jc = feat1 * feat2
        if pred is not None:
...
class MSCDNet(nn.Module):
    ...
    def forward(self,x, x2):
        ...
        hx = self.mfusion4(feat_s5, feat_s4, out_s5)
...
if __name__ == '__main__':
    ...
    print(x2.shape)
    # v1, v2 = model(x1, x2)
    cdout, S1, S2, S3, S4, S5 = model(x1, x2)
    print(cdout.shape)
    print(S1.shape)
    print(S2.shape)
    print(S3.shape)
