...
class DSIFN(nn.Module):
    ...
    def forward(self,t1_input,t2_input):
        ...
        branch_4_out = self.upscore4(branch_4_out)
        branch_3_out = self.upscore3(branch_3_out)
        branch_2_out = self.upscore2(branch_2_out)
        branch_1_out = self.upscore1(branch_1_out)
        return branch_5_out,branch_4_out,branch_3_out,branch_2_out,branch_1_out

if __name__ == '__main__':
    model = DSIFN(vgg16_base(), vgg16_base())
    print(model)
    ...
    print(x2.shape)
    # v1, v2 = model(x1, x2)
    out1, out2, out3, out4, out5 = model(x1, x2)
    print(out1.shape)
    print(out2.shape)
    print(out3.shape)
    print(out4.shape)
